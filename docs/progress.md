...
## Phase 1 — 项目骨架 + 核心模块 ⏸ 未开始

Phase 0 绿了，可以开始搭真正的 app。候选轨道：

- **1a** 项目脚手架：navigation、状态管理、网络层（rcp）、本地存储（`@ohos.data.preferences`）、主题
- **1b** LiveKit 客户端：WebSocket + protobuf v9 信令层 → 接上 `@ohos/webrtc` 的 RTCPeerConnection
- **1c** nacl 补全：`crypto_box_*` + `crypto_sign_*` 两组入口的 NAPI 扩展（authQR 登录必需）
- **1d** `happy-wire` 协议：Zod → ArkTS 类型移植

推荐顺序：**1a 和 1c 可并行**（骨架 vs 小而确定的补强，互不阻塞）。1b 和 1d 依赖 1a 完成。
